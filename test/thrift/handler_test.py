...
class BartenderHandlerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.handler = BartenderHandler(
            self.registry, self.clients, self.plugin_manager, self.request_validator
        )
    ...
    def test_process_request_bad_request(self):
        self.assertRaises(
            bg_utils.bg_thrift.InvalidRequest, self.handler.processRequest, "id"
        )
    ...
    def test_start_instance_not_found(self, plugin_mock):
        ...
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler.startInstance, "instance_id"
        )
    ...
    def test_stop_instance_error(self, plugin_mock):
        plugin_mock.side_effect = bg_utils.bg_thrift.InvalidSystem("", "ERROR MESSAGE")
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler.stopInstance, "instance_id"
        )
        plugin_mock.assert_called_once_with("instance_id")
    ...
    def test_reload_system_not_found(self, system_mock):
        ...
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler.reloadSystem, "id"
        )
    ...
    def test_remove_local_system(self, system_mock):
        ...
        self.clients["pyrabbit"].destroy_queue.assert_has_calls(
            [
                call("request", force_disconnect=False),
    ...
    def test_remove_remote_system(self, system_mock, config_mock):
        ...
        self.clients["pyrabbit"].destroy_queue.assert_has_calls(
            [
                call("request", force_disconnect=True),
    ...
    def test_remove_system_not_found_exception(self, system_mock):
        system_mock.objects.get = Mock(side_effect=mongoengine.DoesNotExist)
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler.removeSystem, "id"
        )
    ...
    def test_clear_queue_404_http_exception(self):
        ...
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler.clearQueue, "queue_name"
        )
        self.clients["pyrabbit"].clear_queue.assert_called_once_with("queue_name")
    ...
    def test_get_plugin_from_instance_id(self, get_system_mock, get_instance_mock):
        ...
        self.assertEqual(
            self.registry.get_plugin.return_value,
        ...
        self.registry.get_plugin.assert_called_once_with(
            self.registry.get_unique_name.return_value
        )

    @patch("bartender.thrift.handler.Instance.objects")
    def test_get_instance(self, objects_mock):
        self.assertEqual(
            objects_mock.get.return_value, self.handler._get_instance(Mock())
        )
    ...
    def test_get_instance_not_found(self, objects_mock):
        ...
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler._get_instance, Mock()
        )

    @patch("bartender.thrift.handler.System.objects")
    def test_get_system(self, objects_mock):
        self.assertEqual(
            objects_mock.get.return_value, self.handler._get_system(Mock())
        )
    ...
    def test_get_system_not_found(self, objects_mock):
        objects_mock.get.side_effect = mongoengine.DoesNotExist
        self.assertRaises(
            bg_utils.bg_thrift.InvalidSystem, self.handler._get_system, Mock()
        )
