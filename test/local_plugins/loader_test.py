...
class PluginLoaderTest(unittest.TestCase):
    ...
    def test_validate_plugin_requirements_no_requirements(self):
        self.mock_registry.get_all_plugins = Mock(return_value=[Mock(
            requirements=[], plugin_name='foo'), Mock(requirements=[], plugin_name='bar')])
        self.loader.validate_plugin_requirements()
        self.assertEqual(self.mock_registry.remove.call_count, 0)

    def test_validate_plugin_requirements_good_requirements(self):
        self.mock_registry.get_all_plugins = Mock(return_value=[Mock(
            requirements=[], plugin_name='foo'), Mock(requirements=['foo'], plugin_name='bar')])
    ...
    @patch('bartender.local_plugins.loader.sys')
    @patch('bartender.local_plugins.loader.load_source')
    def test_load_plugin_config_only_required_attributes(self, load_source_mock, sys_mock):
        module_name = 'BGPLUGINCONFIG'
        sys_mock.modules = {module_name: ''}
    ...
    @patch('bartender.local_plugins.loader.sys')
    @patch('bartender.local_plugins.loader.load_source')
    def test_load_plugin_config_instances_provided_no_plugin_args(self, load_source_mock, sys_mock):
        module_name = 'BGPLUGINCONFIG'
        sys_mock.modules = {module_name: ''}
    ...
    @patch('bartender.local_plugins.loader.sys')
    @patch('bartender.local_plugins.loader.load_source')
    def test_load_plugin_config_plugin_args_dict_no_instances(self, load_source_mock, sys_mock):
        module_name = 'BGPLUGINCONFIG'
        sys_mock.modules = {module_name: ''}
